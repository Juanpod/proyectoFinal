...
export class Categoria {
    ...
    static async getById(idCategoria) {
        const query = 'SELECT idCategoria, nombreCategoria FROM Categorias WHERE idCategoria = ?;'
        let connection
        ...
        try {
            ...
            if (result.length === 0) {
                console.log("Model: No se encontraron datos para el id =", idCategoria)
                return null
            }

            return result[0]
        } catch (error) {
            console.error("Error al obtener la categoría para el id", idCategoria)
    ...
    static async create(nombreCategoria) {
        ...
        const searchQuery = 'SELECT * FROM Categorias WHERE nombreCategoria = ?;'
        let connection

        try {
            connection = await createConnection()
            const [searchResults] = await connection.query(searchQuery, [nombreCategoria])
    ...
    static async update(idCategoria, nombreCategoria) {
        const query = 'UPDATE Categorias SET nombreCategoria = ? WHERE idCategoria = ?;'
        const searchQuery = 'SELECT * FROM Categorias WHERE nombreCategoria = ? AND idCategoria != ?;'
        let connection

        try {
            connection = await createConnection()

            const [searchResult] = await connection.query(searchQuery, [nombreCategoria, idCategoria])
            ...
            const [result] = await connection.query(query, [nombreCategoria, idCategoria])
