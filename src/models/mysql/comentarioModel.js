...
export class Comentario {
    ...
    static async getAll() {
        const query = 'SELECT idComentario, comentario, fechaComentario, idTipoComentario, idTicket FROM Comentarios;';
        let connection;
        try {
    ...
    static async getById(idComentario) {
        const query = 'SELECT idComentario, comentario, fechaComentario, idTipoComentario, idTicket FROM Comentarios WHERE idComentario = ?;';
        let connection;
        try {
            connection = await createConnection();
            const [result] = await connection.query(query, [idComentario]);
            if(result.length === 0){
                console.log("Model: No se encontraron datos para el id=", idComentario)
                return null
            }
    ...
    static async create(comentario, idTipoComentario, idTicket) {
        const query = 'INSERT INTO Comentarios (comentario, fechaComentario, idTipoComentario, idTicket) VALUES (?, NOW(), ?, ?);';
        let connection;
        try {
            connection = await createConnection();

            // Verificar que las claves foráneas existen
            const tipoComentarioExists = await this.tipoComentarioExists(idTipoComentario);
            ...
            const [result] = await connection.query(query, [comentario, idTipoComentario, idTicket]);
    ...
    static async update(idComentario, comentario, idTipoComentario, idTicket) {
        const query = 'UPDATE Comentarios SET comentario = ?, idTipoComentario = ?, idTicket = ? WHERE idComentario = ?;';
        let connection;
        try {
            connection = await createConnection();
            ...
            const [result] = await connection.query(query, [comentario, idTipoComentario, idTicket, idComentario]);
            if (result.affectedRows === 0) {
                console.log("Model: No se encontró el comentario con id=", idComentario)
                return null;
            }
    ...
    static async tipoComentarioExists(idTipoComentario) {
        const query = 'SELECT COUNT(*) AS count FROM TiposComentarios WHERE idTipoComentario = ?;';
        let connection;
        try {
            connection = await createConnection();
    ...
    static async ticketExists(idTicket) {
        const query = 'SELECT COUNT(*) AS count FROM Tickets WHERE idTicket = ?;';
        let connection;
        try {
            connection = await createConnection();
