...
export class Rol {
    ...
    static async getById(idRol) {
        ...
        try {
            ...
            throw new Error(
                "Error en la base de datos. No se pudo obtener el rol."
            );
        } finally {
            if (connection) {
                await connection.end();
    ...
    static async create(nombreRol) {
        ...
        try {
            ...
            const [existingRol] = await connection.query(searchQuery, [
                nombreRol,
            ]); //Query para buscar un rol que ya tenga el nombreRol
            ...
            throw new Error(
                error.message ||
                    "Error en la base de datos. No se pudieron obtener los roles."
            );
        } finally {
            if (connection) {
                await connection.end();
    ...
    static async update(idRol, nombreRol) {
        ...
        const searchQuery =
            "SELECT * FROM Roles WHERE nombreRol = ? AND idRol != ?;";
        ...
        try {
            ...
            const [existingRoles] = await connection.query(searchQuery, [
                nombreRol,
                idRol,
            ...
            if (existingRoles.length > 0) {
                console.log(
                    "Model: El nombre de rol ya existe para otro rol, no se puede actualizar."
                );
                throw new Error("El nombre de rol ya existe para otro rol.");
            }

            console.log(
                "Model: El nuevo nombre para el rol no esta duplicado, se procede"
            );
            ...
            if (result.affectedRows === 0) {
                console.log(
                    "Model: No se encontró el rol con el idRol =",
                    idRol
                );
                return null;
            ...
            throw new Error(
                error.message ||
                    "Error en la base de datos. No se pudo actualizar el rol."
            );
        } finally {
            if (connection) {
                await connection.end();
    ...
    static async delete(idRol) {
        ...
        try {
            ...
            console.error(
                error.message || ("Error al eliminar el rol con id", idRol)
            );
            throw new Error(
                error.message ||
                    "Error en la base de datos. No se pudo obtener el rol."
