...
export class Ticket {
    ...
    static async getById(idTicket) {
        const query =
            "SELECT idTicket, asuntoTicket, descripcionTicket, fechaCreacion, idEstadoTicket, idPrioridad, idCategoria, idUsuarioCreador, idUsuarioResolutor FROM Tickets WHERE idTicket=?;";
        let connection;
        try {
            connection = await createConnection();
    ...
    static async create(
        ...
        const query =
            "INSERT INTO Tickets (asuntoTicket, descripcionTicket, fechaCreacion, idEstadoTicket, idPrioridad, idCategoria, idUsuarioCreador, idUsuarioResolutor) VALUES (?, ?, NOW(), ?, ?, ?, ?, ?);";
        let connection;
        try {
            connection = await createConnection();
            ...
            if (idUsuarioResolutor) {
                const usuarioExists = await this.usuarioExists(
                    idUsuarioResolutor
                );
                if (!usuarioExists) {
                    throw new Error("El idUsuarioCreador no es válido.");
    ...
    static async update(
        ...
        const query =
            "UPDATE Tickets SET asuntoTicket = ?, descripcionTicket = ?, idEstadoTicket = ?, idPrioridad = ?, idCategoria = ?, idUsuarioResolutor = ? WHERE idTicket = ?;";
        let connection;
        ...
        try {
            ...
            if (result.affectedRows === 0) {
                console.log(
                    "Model: No se encontró el ticket con id=",
                    idTicket
                );
                return null;
            }
    ...
    static async estadoExists(idEstadoTicket) {
        const query =
            "SELECT COUNT(*) AS count FROM EstadosTickets WHERE idEstadoTicket = ?;";
        let connection;
        try {
            connection = await createConnection();
    ...
    static async prioridadExists(idPrioridad) {
        const query =
            "SELECT COUNT(*) AS count FROM Prioridades WHERE idPrioridad = ?;";
        let connection;
        try {
            connection = await createConnection();
