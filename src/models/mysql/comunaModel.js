...
export class Comuna {
    ...
    static async getById(idComuna) {
        const query =
            "SELECT idComuna, nombreComuna FROM Comunas WHERE idComuna = ?;";
    ...
    static async update(idComuna, nombreComuna) {
        ...
        const searchQuery =
            "SELECT * FROM Comunas WHERE nombreComuna = ? AND idComuna != ?;";
        ...
        try {
            ...
            if (searchResult.length > 0) {
                console.log(
                    "Model: El nombre de comuna ya existe para otro registro, no se puede actualizar."
                );
                throw new Error("El nombre ya existe.");
            }
