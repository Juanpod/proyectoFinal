...
export class Sucursal {
    ...
    static async getAll() {
        const query = 'SELECT idSucursal, nombreSucursal, direccionSucursal, idComuna FROM Sucursales;'
        let connection
    ...
    static async getById(idSucursal) {
        const query = 'SELECT idSucursal, nombreSucursal, direccionSucursal, idComuna FROM Sucursales WHERE idSucursal = ?;'
        let connection
        try {
            connection = await createConnection()
            ...
            if(result.length === 0){
                console.log("Model: No se encontraron datos para el id=", idSucursal)
                return null
            }
    ...
    static async create(nombreSucursal, direccionSucursal, idComuna){
        const query = 'INSERT INTO Sucursales (nombreSucursal, direccionSucursal, idComuna) VALUES (?,?,?);'
        const searchQuery = 'SELECT nombreSucursal FROM Sucursales WHERE nombreSucursal = ?;'
        let connection
        ...
        try {
            ...
            if (!comunaExists) {
                console.log("Model: La comuna con id =", idComuna, "no existe.");
                throw new Error("El idComuna no es válido.");
            }

            //Verificar si el nombre de la Comuna Existe
            const [searchResults] = await connection.query(searchQuery, [nombreSucursal])
            if(searchResults.length > 0){
                console.log("Model: El nombre ya existe, no se agrega.")
                throw new Error("El nombre ya existe.")
            }

            const [result] = await connection.query(query, [nombreSucursal, direccionSucursal, idComuna])
    ...
    static async update(idSucursal, nombreSucursal, direccionSucursal, idComuna){
        const query = 'UPDATE Sucursales SET nombreSucursal = ?, direccionSucursal = ?, idComuna = ? WHERE idSucursal = ?;';
        const searchQuery = 'SELECT * FROM Sucursales WHERE nombreSucursal = ? AND idSucursal != ?;';
        let connection;
        try {
            connection = await createConnection();
            ...
            const [searchResult] = await connection.query(searchQuery, [nombreSucursal, idSucursal]);
            if (searchResult.length > 0) {
                console.log("Model: El nombre de sucursal ya existe para otro registro.");
                throw new Error("El nombre ya existe.");
            }

            const [result] = await connection.query(query, [nombreSucursal, direccionSucursal, idComuna, idSucursal]);
            if (result.affectedRows === 0) {
                console.log("Model: No se encontró el dato para el id =", idSucursal);
                return null;
            }
    ...
    static async comunaExists(idComuna) {
        const query = 'SELECT COUNT(*) AS count FROM Comunas WHERE idComuna = ?;';
        let connection;
        try {
            connection = await createConnection();
            ...
            throw new Error("Error en la base de datos al verificar la comuna.");
        } finally {
            if (connection) {
                await connection.end();
                console.log("Model: Conexión a la base de datos cerrada.");
