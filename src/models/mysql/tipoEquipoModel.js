...
export class TipoEquipo {
    ...
    static async getById(idTipoEquipo) {
        const query = 'SELECT idTipoEquipo, tipoEquipo FROM TiposEquipos WHERE idTipoEquipo = ?;' // Se define la query
        ...
        try {
            ...
            if (result.length === 0) {
                console.log("Model: No se encontraron datos para el id =", idTipoEquipo);
                return null;
            } 
    ...
    static async create(tipoEquipo) {
        ...
        try {
            connection = await createConnection()
            const [searchResults] = await connection.query(searchQuery, [tipoEquipo])
    ...
    static async update(idTipoEquipo, tipoEquipo){
        const query = 'UPDATE TiposEquipos SET tipoEquipo = ? WHERE idTipoEquipo = ?;' // Se define la query para actualizar el rol por su id
        const searchQuery = 'SELECT * FROM TiposEquipos WHERE tipoEquipo = ? AND idTipoEquipo != ?;'
        ...
        try {
            ...
            const [searchResult] = await connection.query(searchQuery, [tipoEquipo, idTipoEquipo]) // Verificar si el nombre del rol ya existe para otro registro

            if (searchResult.length > 0) {
                console.log("Model: El nombre de tipo de equipo ya existe para otro tipo, no se puede actualizar.")
                throw new Error("El nombre ya existe.")
            }

            console.log("Model: El nuevo nombre no esta duplicado, se procede")
            const [result] = await connection.query(query, [tipoEquipo, idTipoEquipo]) // Se ejecuta la query para actualizar el rol con el id requerido

            
            if (result.affectedRows === 0) {
                console.log("Model: No se encontró el dato para el id =", idTipoEquipo)
                return null
            ...
            console.error("Error al actualizar la entrada para el id", idTipoEquipo);
    ...
    static async delete(idTipoEquipo){
        ...
        try {
            ...
            console.error(error.message || ("Error al eliminar la entrada con id", idTipoEquipo));
            throw new Error(error.message || "Error en la base de datos. No se pudo obtener el dato");
        } finally {  
            if (connection) {
                await connection.end()
                console.log("Model: Conexion a la base de datos cerrada")
