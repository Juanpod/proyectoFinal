...
export class RolController {
    ...
    static async getById (req, res, next) {
        ...
        try {
            console.log(`Controller: Ejecutando metodo getRolById para idRol ${idRol} `)
            ...
            console.log(`Controller: Rol con idRol ${idRol} obtenido exitosamente.`)
        } catch (error) {
            next(error); // Enviar el error al middleware de manejo de errores
        } 
    }
    ...
    static async create (req, res, next) {
        ...
        try {
            console.log(`Controller: Ejecutando el metodo createRol para crear el rol ${nombreRol}`)
            ...
            if (error.message === "El nombre del rol ya existe.") {
                return res.status(409).json({
                    success: false,
                    message: "El nombre del rol ya existe. Por favor elige otro."
                })
            }
          next(error) // Enviar el error al middleware de manejo de errores
        } 
    ...
    static async update(req, res, next) {
        ...
        try {
            console.log(`Controller: Ejecutanto metodo updateRol con idRol=${idRol} y nombreRol=${nombreRol}`)
            ...
            if (error.message === "El nombre de rol ya existe para otro rol.") {
                return res.status(409).json({
                    success: false,
                    message: "El nombre de rol ya existe para otro rol. Por favor elige otro."
                })
            }
            next(error) // Enviar el error al middleware de manejo de errores
        }
    ...
    static async delete (req, res, next) {
        ...
        try {
            console.log(`Controller: Ejecutando el metodo deleteRol para idRol=${idRol}`)
