...
export class ComunaController {
    ...
    static async getById(req, res, next) {
        ...
        try {
            console.log(`Controller: Ejecutando metodo getById para Comuna y id ${idComuna}`)
    ...
    static async create(req, res, next) {
        ...
        try {
            console.log(`Controller: Ejecutando metodo create para crear la comuna ${nombreComuna}`)
            ...
            if (error.message === "El nombre ya existe.") {
                return res.status(409).json({
                    success: false,
                    message: "La comuna ya existe. Por favor elige otro nombre."
                })
            }
            next(error)
        }
    ...
    static async update(req, res, next) {
        ...
        try {
            console.log(`Controller: Ejecutando metodo update para el id=${idComuna} y nombreComuna=${nombreComuna}`);
            ...
            if (error.message === "El nombre ya existe.") {
                return res.status(409).json({
                    success: false,
                    message: "El nombre de la comuna ya existe. Por favor elige otro nombre."
                })
            }
            next(error)
        }
    ...
    static async delete(req, res, next) {
        ...
        try {
            console.log(`Controller: Ejecutando metodo delete para el id=${idComuna}`);
