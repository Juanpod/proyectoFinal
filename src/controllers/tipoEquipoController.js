...
export class TipoEquipoController {
    ...
    static async getAll(req, res, next){
        try {
            console.log("Controller: Ejecutando metodo getAll para TiposEquipos")
    ...
    static async getById(req, res, next) {
        ...
        try {
            console.log(`Controller: Ejecutando metodo getById para Tipo de equipo y id ${idTipoEquipo} `)
            ...
            console.log(`Controller: Dato con id ${idTipoEquipo} obtenido exitosamente.`)
        } catch (error) {
    ...
    static async create(req, res, next){
        ...
        try {
            console.log(`Controller: Ejecutando el metodo create para crear el tipo de equipo ${tipoEquipo}`)
            ...
            if (error.message === "El nombre ya existe.") {
                return res.status(409).json({
                    success: false,
                    message: "El tipo de equipo ya existe. Por favor elige otro."
                })
            }
            next(error);
        }
    ...
    static async update(req, res, next){
        ...
        try {
            console.log(`Controller: Ejecutanto metodo update para el id=${idTipoEquipo} y tipo de equipo=${tipoEquipo}`)
    
            const updatedEntry = await TipoEquipo.update(idTipoEquipo, tipoEquipo) // Actualizar el rol con el método del modelo
            ...
            console.log("Controller: El resultado del update es:", updatedEntry)
        } catch (error) {
            if (error.message === "El nombre ya existe.") {
                return res.status(409).json({
                    success: false,
                    message: "El nombre de tipo de equipo ya existe. Por favor elige otro."
                })
            }
            next(error) // Enviar el error al middleware de manejo de errores
        }
    ...
    static async delete(req, res, next){
        ...
        try {
            console.log(`Controller: Ejecutando el metodo delete para Tipo de Equipos y id=${idTipoEquipo}`)
